...
def create_atom_xml(directory_path, output_file):
    ...
    for filename in os.listdir(directory_path):
        if filename.endswith('.html'):
            ...
            entry_link = ET.SubElement(entry, 'link', href=f'https://gregoirelits.eu/{filename}')

            entry_id = ET.SubElement(entry, 'id')
            entry_id.text = f'https://gregoirelits.eu/{filename}'
        ...
        # Utiliser une expression régulière ou un analyseur HTML pour extraire le contenu après la balise <article>
        # Supposons que vous utilisez une expression régulière pour simplifier l'exemple
        import re
